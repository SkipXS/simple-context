...
const instructions = "Prefer these bounded tools to keep context small: sc-run for short shell commands where compact stdout is the result; sc-logs for verbose diagnostic commands across any ecosystem (tests, builds, lints, typechecks, publishes, CI commands, server/runtime logs such as dotnet test/build/publish, npm/pnpm/yarn test/build/lint, vite/next/tsc/eslint/jest/vitest/playwright, cargo test/build/check, go test, pytest, mvn/gradle test/build, Android gradle tasks/adb logcat, xcodebuild/swift/fastlane, docker/kubectl logs); sc-validate to auto-select one project validation command; sc-read for file previews; sc-search for local text/AST search; sc-discover before broad repo reads; sc-fetch for readable web pages; sc-diff for git status/diff/history; sc-env for environment/toolchain preflight; and sc-usage for savings/guidance. "
